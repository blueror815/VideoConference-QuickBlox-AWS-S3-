...
function retrieveChatDialogs() {
  ...
  QB.chat.dialog.list(null, function(err, resDialogs) {
    if (err) {
      ...
      console.log("zzzzzzz", resDialogs);
      ...
      updateDialogsUsersStorage(jQuery.unique(occupantsIds), function(){
        ...
        //
        /*
        _id                         : "5640ad87a28f9a562f0002bb"
        created_at                  : "2015-11-09T14:28:23Z"
        last_message                : "gfhjhg"
        last_message_date_sent      : 1450012417
        last_message_user_id        : 6729119
        name                        : null
        occupants_ids               : [6729114, 6729119]
        photo                       : null
        silent_ids                  : []
        type                        : 3
        unread_messages_count       : 0
        updated_at                  : "2015-12-13T13:13:37Z"
        user_id                     : 6729114
        xmpp_room_jid               : null
        */
        ...
        if(total_unread_msg_count > 0) {
          $("#total-unread-msg").html(total_unread_msg_count).fadeIn(200); 
        } else {
          $("#total-unread-msg").html("0").fadeOut(100);
        }
...
function showOrUpdateDialogInUI(itemRes, updateHtml) {
  ...
  if (dialogType == 3) {
    ...
    opponentName  = getUsernameById(opponentId);
  ...
  if (updateHtml === true) {
    ...
    var dialogHtml = "<li><a href='#' qb_id='" + opponentId + "' qb_login='" + opponentLogin + "'>" 
    + opponentName;
    if(dialogUnreadMessagesCount > 0) {
      dialogHtml += '<span id="unread-msg" class="badge badge-warning">' + dialogUnreadMessagesCount + '</span>';
    }
    dialogHtml += "</a></li>";
    
    total_unread_msg_count += dialogUnreadMessagesCount;
    $('#msg_chat_dropdown').append(dialogHtml);
  }
}
...
function triggerDialog(dialogId){
  ...
  $('.badge').text(0).delay(250).fadeOut(500);
